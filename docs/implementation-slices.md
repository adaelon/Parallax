...
状态：实施中；S01～S03 已于 2026-07-29 完成

本方案以 [产品需求](product-spec.md)、[目标架构](architecture.md)、[领域语言](../CONTEXT.md) 和 `docs/adr/` 中的全部决策为约束。仓库当前尚无可执行实现，因此路线从最小领域闭环开始，逐步完成安全存储、资料摄取、检索与记忆、关系与身份、活动采集、恢复和首版验收。

## 1. 结论

完整首版拆成 31 个可独立验收的纵向切片，而不是只做原方案中的三个核心切片：
...
| G08 主动回顾频率、免打扰规则与资源预算 | S26 | 调度状态机和时间测试夹具 |
| G09 备份保留、删除状态重放和密钥轮换 | S30 | 恢复矩阵和删除复活攻击用例 |
| G10 真实个人资料基准和隐私安全测试方法 | S31 | 脱敏基准、验收脚本、纵向观察模板 |
...
**状态**：实施中；G04 与 thin Tauri 宿主已实现为单实例优先注册、当前用户 `--background` 自启动、强制签名升级、30 秒加密心跳与失败仍继续清理的显式退出状态机；持续对话组件留在后续子片。
...
**主决策**：ADR-0008、ADR-0011、ADR-0012、ADR-0026、ADR-0037、ADR-0049。
...
**新增/输出**：把 [产品需求第 6 节](product-spec.md) 的确定性判据固化为自动化验收套件，并执行至少两周纵向试用；覆盖模型切换、重启、资料变化、纠错、争议、遗忘、恢复和身份连续。

**明确不做**：用主观“像不像本人”替代测试、宣称主观意识连续、扩大首版威胁承诺或文件格式范围。

**确定性完成**：全部自动验收、威胁边界测试和迁移测试通过；纵向报告确认身份、观点来源和纠错效果；失败项回到对应切片修复，不在发布片里打补丁掩盖。
...
S31 完成前，任一 accepted ADR 没有通过其主切片测试、任一 FR 没有进入系统验收，首版都不得宣称完成。
