...
10. 首个身份与首个自我包未完整持久化时不得进入正式对话；普通运行时请求不存在无身份降级路径。
...
**状态**：已完成；当时 50 个 accepted ADR 与 S06R 必需证据矩阵全绿，Full runner 18/18 并冻结候选构建；该构建已被 S06D 与 S07C 后续变更作废。
...
**确定性完成**：相关定向测试与 `scripts/run-system-acceptance.ps1 -Mode Full` 全绿；安装、首次创建、配置、热切换、重启和卸载烟测通过；该历史时点曾打开 S32，现由 S07C 重新锁定。
...
CounterpartReadiness =
  ...
  | READY { identity_version, self_bundle_version }
  | INCONSISTENT { reason }

send_message(message):
  require CounterpartReadiness::READY
  require current identity and Self Bundle agree on identity_version
  append person evidence
  invoke ordinary runtime with required counterpart state
  append counterpart evidence bound to identity_version
```

旧保险库中的无身份回复继续作为逐字证据保留，但标记为未绑定身份，不追溯归属给后来形成的第二自我，也不进入其默认关系历史、判断依据或身份演化依据。

#### S07C-1 真实初始身份运行时契约

**状态**：已完成；真实 Runtime Gateway 已实现 Responses/DeepSeek 等价的严格初始身份契约。

**依赖/输入**：S04 `InitialIdentityRequest`、S06D 双协议适配、六类初始自我介绍的合成夹具。

**新增/输出**：为真实 Runtime Gateway 实现 `IdentityRuntime::form_initial_identity`；Responses 与 DeepSeek 使用同一严格 schema，输出六个身份字段、形成理由和仅来自初始介绍的证据引用；外发记录固定标识身份形成调用。

**明确不做**：Vault 写入、Self Bundle 创建、桌面 command、自由文本身份解析或普通对话回应。

**确定性完成**：两种协议的固定响应产生等价 `InitialIdentityProposal`；缺字段、额外字段、冒充本人、放弃反思使命、越界证据和提示注入夹具均失败关闭；外发只含六类介绍证据。

**主决策**：ADR-0001、ADR-0004、ADR-0039、ADR-0045、ADR-0048、ADR-0054、ADR-0055。
...
**新增/输出**：增加一个可信创建事务，在同一提交中写入 `IdentityStateVersion(1)` 与 `SelfBundleVersion(1)`；首个自我包固定引用身份 v1，以已校验的关系姿态初始化关系状态，并以空经历、信念和未完成意图开始；Repository 派生 `CounterpartReadiness`。

**明确不做**：初始介绍与身份合并成角色卡、自动生成长期记忆、创建后身份修订、宿主 command 或 UI。

**确定性完成**：缺介绍、已有任一首版、身份/自我包版本不一致均拒绝；在父行和任一子项注入失败后重启只能看到零个或完整一对首版；重复请求不产生第二个版本。

**主决策**：ADR-0002、ADR-0039、ADR-0045、ADR-0055。

#### S07C-3 正式对话门禁与回复身份归属

**状态**：已完成；Core 在任何本人证据或运行时调用前从持久化事实复核 `READY` 及身份/自我包版本，普通运行时请求不再允许缺失身份，新回复以 schema v27 持久绑定产生它的身份版本，旧回复迁移为身份前未绑定且不能进入默认认识链路。

**依赖/输入**：S07C-2 的 `CounterpartReadiness` 和原子首版、现有 `MemoryCore::run_counterpart_turn`。

**新增/输出**：普通对话请求把身份和自我包版本改为必填；Core 在保存本人消息前复核 `READY`；每条新第二自我回复持久绑定产生它的身份版本。迁移把既有无版本回复投影为 `PRE_IDENTITY_UNBOUND`，默认认识链路只接受 `IDENTITY_BOUND(version)`。

**明确不做**：删除或改写旧对话、把旧回复归给身份 v1、改变检索排序、传递完整 Self Bundle 内容或修改回答策略。

**确定性完成**：四种非 `READY` 状态调用 `send_message` 都零证据、零运行时调用、零账本写入；正常回复绑定当前身份版本；旧回复重启后仍逐字可审计但不能支持第二自我判断、共同经历或身份修订。

**主决策**：ADR-0001、ADR-0026、ADR-0027、ADR-0038、ADR-0055。
...
**新增/输出**：用有界 `CounterpartSelfContext` 替代“身份可选 + 自我包只有版本号”的出口；内容包括当前身份、关系状态、经 Core 解析的活动信念、未完成意图和与本轮相关的第二自我经历，并保留宪法与版本边界。

**明确不做**：把整个 Vault 或全部历史发送给模型、让运行时自行解引用 Claim ID、用摘要覆盖权威证据、在本片调提示词风格。

**确定性完成**：严格请求夹具可观察全部选中主体状态；悬空信念、错误身份版本或越预算状态失败关闭；未选证据和无关个人资料不出现在请求体或外发记录中；模型切换得到同一主体状态。

**主决策**：ADR-0002、ADR-0004、ADR-0039、ADR-0055。
...
**状态**：已完成；React 在 Vault 就绪后先读取宿主派生的四态 readiness，分别路由六类介绍、可重试身份形成、失败关闭页与正式聊天；会话只读投影显式携带回复身份归属，旧回复仅标作“创建前记录”。

**依赖/输入**：S07C-5 的固定只读状态与创建 commands、现有 React 恢复流程。

**新增/输出**：Vault 解锁后按 `CounterpartReadiness` 路由：未介绍时显示六类最小介绍，已记录时显示可重试的形成步骤，`READY` 后才显示正式聊天；创建完成展示第二自我的名字和首版关系姿态，但不提供身份编辑控件。旧的未绑定回复显示为“创建前记录”。

**明确不做**：完整人生问卷、第二自我角色卡编辑、跳过介绍按钮、把运行时错误正文回显、隐藏或删除旧证据。

**确定性完成**：React 测试覆盖六类缺失、提交成功、形成失败重试、重启恢复、READY 路由、输入框门禁、创建前记录标识、键盘/焦点行为，以及保险库与创建页面的视口有界滚动；类型检查、生产构建和桌面/窄屏真实滚动回归通过。

**主决策**：ADR-0001、ADR-0045、ADR-0055。

#### S07C-7 普通对话本人事实原子化

**状态**：已完成；Responses 与 DeepSeek 均产出最多 32 项原子本人事实提议，Core 逐项失败关闭，多个 Claim 的归属、时间和精确引用已通过 SQLCipher 重启验证。

**依赖/输入**：S07C-3 的身份绑定对话证据、S01 本人事实分类链路。

**新增/输出**：把单个粗粒度分类结果改为 `0..N` 个带精确原文引用和适用时间的本人事实提议；Core 独立校验每项归属、引用与非空原子陈述，同一原始发言仍只保存一条对话证据。

**明确不做**：从寒暄、问题、假设、引用或玩笑推断事实；把事实自动晋升为长期记忆；在本片发现跨时间模式。

**确定性完成**：固定夹具证明“你好”产生零事实，多项身份/经历自述拆成多个 Claim，同一事实重复不靠复制文本凑记录，含糊混合语句只接收可逐字支持的原子项；重启后归属和引用保持一致。

**主决策**：ADR-0003、ADR-0025、ADR-0026、ADR-0055。

#### S07C-8 反思回应契约与非自评验收

**状态**：已完成；自动化、静态门禁与本人四项语言验收均通过。

**A1 切片声明**：冻结普通与高影响回应的任务优先、独立立场、证据/不确定性、可反驳行动与单例反模式边界；不新增人格标签、运行时自评或持久化旁路。完成以 Runtime/Core 自动化门禁、四组脱敏固定场景的本人四项验收、受影响包与全仓格式检查全部通过为准。

| 验收项 | 确定性判据 | 当前结果 |
| --- | --- | --- |
| 回应请求 | 普通/高影响指令都含任务优先、独立选择、证据与不确定性、可验证行动 | Runtime 43/43、workspace 347/347 通过 |
| 持久化边界 | 自由文本只作对话证据；持久解释只经现有判断/记忆路径 | Core 定向测试通过 |
| 单例边界 | 单次表现不能生成重复模式或新人格标签操作 | Core/Runtime 定向测试通过 |
| 操作白名单 | schema 无自评/人格标签字段，未知人格标签操作只形成拒绝结果 | Runtime 定向测试通过 |
| 四组固定场景 | 技术信心、关系矛盾、目标选择、普通事务均保留同一主体输入和客观 contract | Runtime 43/43、链接/隐私/静态门禁通过 |
| 本人语言验收 | 独立立场、证据与不确定性、可被反驳、任务优先逐项记录 | 2026-08-09 本人确认“四项通过” |

**依赖/输入**：S07C-4 主体状态、S07C-7 原子事实、S25～S27 现有身份修订、判断、反思和模式机制。

**新增/输出**：普通回应契约要求先完成本人当前任务，再基于身份和证据选择赞同、质疑、暂定解释、追问反例或提出可验证行动；持久解释继续走现有判断/记忆提议，不新增人格标签。建立技术信心、关系矛盾、目标选择和普通事务四组脱敏场景。

**明确不做**：强制每轮反驳、把所有对话治疗化、用无依据安慰替代判断、让 LLM 为自己的回答打分、把一次表现概括为人格模式。

**确定性完成**：自动化测试锁定身份/主体状态输入、来源引用、结构化操作和反例门槛；固定场景由本人按“独立立场、证据与不确定性、可被反驳、任务优先”四项人工验收并记录结果，不使用 LLM-as-judge 作为通过依据。

**主决策**：ADR-0027、ADR-0035～ADR-0044、ADR-0055。
...
**新增/输出**：为第二自我就绪门禁、首版原子创建、回复身份归属、主体状态出口、桌面创建流程和事实原子化建立新的系统证据 ID；更新 accepted ADR/FR/DET/迁移矩阵，重跑 Full runner，生成并记录新的 NSIS 安装包与 SHA-256。

**明确不做**：沿用任何旧冻结构建、把人工语言评价伪装成确定性测试、缩短 S32 十四日观察、在验收片新增产品行为。

**确定性完成**：定向测试、`cargo test --workspace --no-fail-fast`、桌面测试/类型/构建、Validate、Full 与安装生命周期烟测全部通过；ADR-0055 有可执行证据；S32 模板只指向同一个新构建并从零开始观察。
