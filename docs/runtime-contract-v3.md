...
本契约在 v2 的可配置运行时目标上增加受限的 DeepSeek Chat Completions 协议适配，并纳入 S07C-1 的真实初始身份调用。协议适配只改变运行时后端的 wire encoding；Core/Identity 端口、最小数据出口、领域 JSON schema、操作白名单、凭据边界和失败关闭语义保持不变。
...
3. 所有非 DeepSeek 目标继续产生逐字段相同的 Responses 请求和领域结果。
4. DeepSeek 官方 Base URL 只产生一个 `/chat/completions`，请求含 `messages`、`json_object` 和关闭思考字段，不含 Responses-only 字段。
5. 等价 DeepSeek fixture 产生与 Responses fixture 相同的 `PersonTurnClassification` 和 `RuntimeResponse`。
6. DeepSeek 缺失 content 或非 `stop` 完成原因返回 `InvalidResponse`。
