...
PersonMessageReceived
  -> rederive CounterpartReadiness from persisted introduction + identity + Self Bundle
...
ConversationStarted | EvidenceChanged | ScheduledReflection | ImportantChange
  ...
  -> invoke_runtime(required identity_state + self_bundle_version,
                    constitution, active_relational_constraints, working_context)
  -> express response in counterpart identity and relationship voice
       complete the person's current task before reflection unless immediate safety risk
       choose an independent stance: agree | challenge | provisional interpretation |
                                     ask for counterevidence | verifiable action
       distinguish supplied evidence from inference; state material uncertainty
       keep interpretations revisable and name evidence that could change them
       never generalize one performance into a pattern or personality label
       never ask the runtime or another model to score the response
  ...
  -> append verbatim ConversationEvidence(
       speaker=counterpart,
       attribution=IDENTITY_BOUND(identity_version))
  -> validate_structured_outputs()
       reject PRE_IDENTITY_UNBOUND evidence as support for judgments,
       shared relationship history or identity revisions
...
S07 的持续对话只通过两个白名单 command 接通，不把 Core 能力面传给 WebView：
...
send_message(verbatim)
  ...
  -> MemoryCore::run_counterpart_turn 先复核 READY 与当前版本
  -> 非 READY 或版本不一致时零证据、零运行时调用、零账本写入
  -> 返回本人与第二自我的两个逐字证据视图
```

S07C-5 在同一白名单和宿主锁内补齐创建入口：

```text
get_counterpart_readiness()
  -> 从介绍、身份与 Self Bundle 持久化事实派生四态
  -> 只返回状态、READY 版本与固定 INCONSISTENT 原因码

record_initial_self_introduction(six_named_fields)
  -> require NEEDS_INTRODUCTION
  -> IdentityFormation 原子提交六条本人证据与事实
  -> return INTRODUCTION_RECORDED；不回显介绍正文

form_initial_counterpart()
  -> require INTRODUCTION_RECORDED
  -> 复用当前活动 IdentityRuntime 形成严格结构化提议
  -> Identity/Vault 原子提交 identity v1 + Self Bundle v1
  -> return READY；运行时/解析/领域/存储错误压缩为固定类别

send_message(verbatim)
  -> ManagedHost 在检索前 require READY
...
run_counterpart_turn(prompt, WorkingContext)
  ...
  -> reject PRE_IDENTITY_UNBOUND support from judgment/shared/identity knowledge paths
...
MemoryRepository append/read
  -> evidence rows retain verbatim text, speaker, session, recorded time and reply identity version
  -> schema v27 maps legacy counterpart NULL attribution to PRE_IDENTITY_UNBOUND
...
`cipher_memory_security` 未启用：G01 的 Windows spike 证明 `VirtualLock` 配额失败会导致崩溃；关闭契约改为先释放 SQLCipher 的连接内密钥，再显式清零 Rust 持有的 Vault Key。S02 先用 HKDF 与 XChaCha20-Poly1305 固定向量锁定兼容性，S08 再落地对象文件与归档引用。S07C-3 的 v27 migration 只增加回复归属列与索引；旧回复不改写、不删除，也不追溯绑定后来身份。该实现落实 [ADR-0009](adr/0009-hybrid-encrypted-vault-storage.md)、[ADR-0011](adr/0011-trust-current-windows-logon-session.md)、[ADR-0026](adr/0026-retain-every-conversation-turn-as-evidence.md) 和 [ADR-0046](adr/0046-vault-cryptographic-profile.md)。
...
crates/runtime-gateway/src/
  ...
  adapter.rs            # 初始身份/本人事实提议/任务优先反思回应的双协议编码、严格 schema 与精确外发记录
...
OpenAiResponsesRuntime::respond(RuntimeRequest)
  ...
  -> require task-first independent stance from identity + evidence
  -> separate evidence/inference, expose uncertainty and a falsifiable next step
  -> forbid forced disagreement, therapy-by-default, unsupported reassurance,
            single-performance patterns/personality labels and runtime self-scoring
  -> adapt and record exact provider request without credentials
  -> extract protocol-specific structured JSON text
  -> parse free text, citations and the unchanged structured-operation whitelist
  -> persistent interpretation may use propose_judgment; free text remains evidence only
...
ManagedHost::save_runtime_profile(draft)
  -> under the same mutex, validate/build candidate runtime
...
```text
first process -> single-instance plugin -> unlock Vault -> load RuntimeProfile -> build runtime -> begin_host_session -> tray event loop
